...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        #hidden_error = self.weights_hidden_to_output*error*hidden_outputs*(1 - hidden_outputs)
        ...
        hidden_outputs = np.reshape(hidden_outputs,(np.product(hidden_outputs.shape),1)) 
        hidden_error_term =  hidden_error*hidden_outputs*(1 - hidden_outputs)
        #print(' shape hidden error term',np.shape(hidden_error_term))
        
        # TODO: Add Weight step (input to hidden) and Weight step (hidden to output).
        # Weight step (input to hidden)
        X = np.reshape(X,(np.product(X.shape),1)) 
        delta_weights_i_h += self.lr*np.dot(X,hidden_error_term.T)
